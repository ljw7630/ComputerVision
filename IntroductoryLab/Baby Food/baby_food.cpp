...
IplConvKernel * arrayOfConvKernelElements[4];
int elementindex=0;

void createStructuringElement()
{
	arrayOfConvKernelElements[0] = NULL;
	arrayOfConvKernelElements[1] = cvCreateStructuringElementEx(3, 3, 1, 1, MORPH_RECT, NULL);
	arrayOfConvKernelElements[2] = cvCreateStructuringElementEx(3, 3, 1, 1, MORPH_ELLIPSE, NULL);
	arrayOfConvKernelElements[3] = cvCreateStructuringElementEx(3, 3, 1, 1, MORPH_CROSS, NULL);
}
...
int find_spoons( IplImage* source, IplImage* result, IplImage* temp)
{
	...
	// jinwu
	cvMorphologyEx( result, temp, NULL, arrayOfConvKernelElements[elementindex], CV_MOP_OPEN, 3 );
	cvMorphologyEx( temp, result, NULL, arrayOfConvKernelElements[elementindex], CV_MOP_CLOSE, 3 );
...
int main( int argc, char** argv )
{
	...
	do {
		...
		// jinwu
		if((user_clicked_key>='a') && (user_clicked_key<='d'))
		{
			elementindex = user_clicked_key-'a';
		}
